...
This is the core realmail library, composed by the following parts
...
```sh
$ npm install --save realmail-core
```

or

```sh
$ yarn add realmail-core
```

## transform mjml to json

```ts
import { MjmlToJson } from 'realmail-core';
...
const myFirstBlock = createBlock({
  name: 'My first block',
  type: 'MY_FIRST_BLOCK',
  create(payload) {
