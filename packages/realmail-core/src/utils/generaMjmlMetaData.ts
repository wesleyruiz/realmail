        ...
        const value = isMultipleAttributes
          ? Object.keys(values[attKey]!)
            .map((childKey) => {
              ...
              return `${childKey}="${isString(childValue)
            ...
            .join(' ')
          : `${key}="${values[attKey]}"`;
        return `<mj-html-attribute class="real-email" multiple-attributes="${isMultipleAttributes}" attribute-name="${key}" ${value}></mj-html-attribute>`;
      })
      .join('\n')}
